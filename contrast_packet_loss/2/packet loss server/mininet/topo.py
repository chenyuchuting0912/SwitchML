...
class MyTopo(Topo):
    def __init__(self, sw_path, json_path, nb_hosts, nb_switches, links, **opts):
        ...
        for a, b in links:
            #self.addLink(a, b)
            if a=='h3' and b=='s1':
                self.addLink(a, b, cls=TCLink, bw=10, delay='40ms', loss=0, max_queue_size=1000, use_htb=True)
            else:
                self.addLink(a, b, cls=TCLink, bw=10, delay='10ms', loss=0, max_queue_size=1000, use_htb=True)
