...
def retrans(idx):
    ...
    for i in range(len(re_sent_vector)):
        re_sent_packet += struct.pack("f", re_sent_vector[i])
    ...
    timers[idx-1]=threading.Timer(0.1,retrans,args=[idx,])
    timers[idx-1].start()
    recounts +=1
...
while True:
    ...
    p[recv_i-1].off=int(struct.unpack("i",recv_packet[12:16])[0])
    ...
    if p[recv_i-1].off<len(sent_data) and p[recv_i-1].off+k <= len(sent_data):
        ...
        ss.sendto(sent_packet, sent_addr)
        timers[recv_i-1]=threading.Timer(1,retrans,args=[recv_i])
